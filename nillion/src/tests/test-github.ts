...
async function testGithub() {
  ...
  try {
    ...
    const projectId1 = await githubService.createProject(
      ...
      "Uses OpenAI GPT-4, implements RAG with vector database",
      "https://github.com/user/ai-chat-assistant",
      "john_doe",
      JSON.stringify(["jane_smith", "bob_wilson"]),
    ...
    const projectId2 = await githubService.createProject(
      ...
      "Smart contracts in Solidity, Web3 frontend",
      "https://github.com/user/blockchain-voting",
      "alice_crypto",
      JSON.stringify(["charlie_dev"]),
