...
async function advancedAnalytics() {
    ...
    try {
        ...
        const [logs, projects, leads, stories, socials, grants, compliance] = await Promise.all([
        ...
        const projectStats = projects.map((project) => {
            const projectLogs = logs.filter((log) => log.data.repoUrl === project.repo_url);
            ...
            return {
                ...
                agentActivity,
                totalActivity: Object.values(agentActivity).reduce((sum, count) => sum + count, 0),
            };
        });
        projectStats.forEach((project) => {
            const activityScore = project.totalActivity;
            const successScore = project.stars + project.collaborators * 10;
            console.log(`  ${project.name}: Activity=${activityScore}, Success=${successScore}`);
        });
        ...
        const leadSourceAnalysis = leads.reduce((acc, lead) => {
            ...
            if (interestLevel === "high")
                acc[source].highInterest++;
            // Extract budget numbers (simplified)
            const budgetMatch = budgetRange === null || budgetRange === void 0 ? void 0 : budgetRange.match(/\$(\d+)k/);
            if (budgetMatch) {
        ...
        Object.entries(leadSourceAnalysis).forEach(([source, stats]) => {
            const conversionRate = ((stats.highInterest / stats.count) * 100).toFixed(1);
            const avgBudget = (stats.totalBudget / stats.count).toFixed(0);
            console.log(`  ${source}: ${stats.count} leads, ${conversionRate}% high interest, $${avgBudget} avg budget`);
        });
        ...
        if (socialAnalysis.twitter) {
            console.log(`  Twitter: ${socialAnalysis.twitter.accounts} accounts, ${socialAnalysis.twitter.totalActions} total actions`);
            Object.entries(socialAnalysis.twitter.actionBreakdown).forEach(([action, count]) => {
                console.log(`    ${action}: ${count}`);
            });
        }
        if (socialAnalysis.telegram) {
            console.log(`  Telegram: ${socialAnalysis.telegram.bots} bots, ${socialAnalysis.telegram.totalMessages} messages`);
            console.log(`    Avg messages per bot: ${(socialAnalysis.telegram.totalMessages / socialAnalysis.telegram.bots).toFixed(1)}`);
        }
        // Grant success indicators
        console.log("\n🎯 Grant Performance Metrics:");
        const grantAnalysis = grants.reduce((acc, grantCollection) => {
            ...
            grantCollection.grants.forEach((grant) => {
                ...
                const daysSinceApplication = (Date.now() - grant.applied_at) / (1000 * 60 * 60 * 24);
                const milestonesPerDay = grant.milestones.length / Math.max(daysSinceApplication, 1);
        ...
        if (grantAnalysis.totalGrants > 0) {
            const avgMilestonesPerGrant = (grantAnalysis.totalMilestones / grantAnalysis.totalGrants).toFixed(1);
            const avgProductivity = grantAnalysis.productivityScores.reduce((sum, score) => sum + score, 0) / grantAnalysis.productivityScores.length;
            ...
            console.log(`  Avg Milestone Completion Rate: ${avgProductivity.toFixed(4)} milestones/day`);
        }
        ...
        Object.entries(complianceRisk).forEach(([regulation, data]) => {
            ...
            console.log(`  ${regulation}: ${complianceRate}% compliant (${data.total} records)`);
        ...
        Object.entries(agentWorkload).forEach(([agent, hours]) => {
            const peakHour = Object.entries(hours).reduce((peak, [hour, count]) => count > (hours[peak] || 0) ? parseInt(hour) : peak, 0);
            console.log(`  ${agent}: Peak at ${peakHour}:00 (${hours[peakHour]} actions)`);
        });
