...
async function testSocials() {
  ...
  try {
    ...
    const twitterId = await socialsService.createTwitterAccount(
      "ai_startup_bot",
      "bot@aistartup.com",
      "super_secret_password_123"
    );

    const telegramId = await socialsService.createTelegramBot(
      "BlockchainNewsBot",
      "1234567890:ABCdefGHIjklMNOpqrSTUvwxYZ123456789"
    );

    console.log(
      `✅ Created social accounts: Twitter=${twitterId}, Telegram=${telegramId}\n`
    );

    // Add Twitter actions
    console.log("🐦 Adding Twitter actions...");
    await socialsService.addTwitterAction(
      twitterId,
      "tweet",
      "Just launched our new AI-powered analytics platform! 🚀 #AI #Analytics #Tech"
    );

    await socialsService.addTwitterAction(
      twitterId,
      "retweet",
      "Great insights on the future of AI!",
      "tweet_123456"
    );
    ...
    await socialsService.addTwitterAction(
      twitterId,
      "reply",
      "Thanks for sharing! We completely agree with your perspective.",
      "tweet_789012"
    );

    // Add Telegram messages
    console.log("💬 Adding Telegram messages...");
    await socialsService.addTelegramMessage(
      telegramId,
      "user_001",
    ...
    if (allSocials.length > 0) {
      ...
      if (social.telegram) {
        ...
        if (social.telegram.messages.length > 0) {
          ...
          console.log(
            `    Latest Message: "${message.text}" from ${message.user_id}`
          );
        }
      }
    }
