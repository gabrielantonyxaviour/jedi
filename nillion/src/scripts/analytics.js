...
async function runAnalytics() {
    ...
    try {
        const logsService = index_js_1.ServiceFactory.getLogs();
        const githubService = index_js_1.ServiceFactory.getGithub();
        const leadsService = index_js_1.ServiceFactory.getLeads();
        ...
        if (allProjects.length > 0) {
            ...
            console.log(`Average Stars per Project: ${(totalStars / allProjects.length).toFixed(1)}`);
        }
        console.log("\n=== LEADS ANALYTICS ===");
        ...
        if (allStories.length > 0) {
            ...
            console.log(`Original vs Remix: ${allStories.length - remixCount} original, ${remixCount} remixes`);
            const avgOwners = allStories.reduce((sum, story) => sum + story.owners.length, 0) /
                allStories.length;
        ...
        if (allSocials.length > 0) {
            ...
            console.log(`Twitter Accounts: ${twitterAccounts} (${totalTwitterActions} total actions)`);
            console.log(`Telegram Bots: ${telegramBots} (${totalTelegramMessages} total messages)`);
            console.log(`LinkedIn Accounts: ${linkedinAccounts}`);
            if (twitterAccounts > 0) {
                console.log(`Average Twitter Actions per Account: ${(totalTwitterActions / twitterAccounts).toFixed(1)}`);
            }
            if (telegramBots > 0) {
                console.log(`Average Messages per Telegram Bot: ${(totalTelegramMessages / telegramBots).toFixed(1)}`);
            }
        }
        console.log("\n=== GRANTS ANALYTICS ===");
        ...
        if (allGrantCollections.length > 0) {
            ...
            console.log(`Average Grants per Collection: ${(totalGrants / allGrantCollections.length).toFixed(1)}`);
            console.log(`Average Members per Collection: ${(totalMembers / allGrantCollections.length).toFixed(1)}`);
            if (totalGrants > 0) {
                console.log(`Average Milestones per Grant: ${(totalMilestones / totalGrants).toFixed(1)}`);
            }
        }
