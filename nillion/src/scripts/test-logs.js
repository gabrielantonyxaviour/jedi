...
async function testLogs() {
    ...
    try {
        ...
        const logId1 = await logsService.createLog(projectId, "github", "Successfully analyzed repository structure", {
            repoUrl: "https://github.com/test/repo",
            fileCount: 42,
            language: "TypeScript",
        });
        const logId2 = await logsService.createLog(projectId, "leads", "Identified 5 potential leads from social media", {
            platform: "twitter",
            leads: ["@user1", "@user2", "@user3"],
            sentiment: "positive",
        });
        const logId3 = await logsService.createLog(projectId, "orchestrator", "Coordinating next steps between agents", { nextActions: ["analyze_code", "contact_leads"], priority: "high" });
