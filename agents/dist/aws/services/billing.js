...
export class BillingService {
    ...
    // =============================================================================
    async recordCreditConsumption(consumption) {
        ...
        await this.updateProjectFinancials(consumption.projectKey, consumption.usdConsumed);
        console.log(`💰 Recorded ${consumption.usdConsumed} USD consumption for ${consumption.projectKey} - ${consumption.action}`);
    }
    async recordGitHubAnalysis(projectKey, repoUrl, workflowId) {
        ...
        const estimatedCost = this.AWS_COSTS.GITHUB_API_CALL * 75 + // ~75 API calls
    ...
    async recordWebhookProcessing(projectKey, repoName, commitsCount) {
        ...
        const estimatedCost = this.AWS_COSTS.GITHUB_API_CALL * commitsCount + // API calls per commit
    ...
    async recordBulkAnalysis(projectKey, repoCount, workflowId) {
        // Bulk analysis scales with repository count
        const singleAnalysisCost = this.AWS_COSTS.GITHUB_API_CALL * 75 +
    ...
    async recordProjectLaunchWorkflow(projectKey, enabledAgents, workflowId) {
        ...
        const coordinationCost = this.AWS_COSTS.SQS_REQUEST * 0.01 + // Multiple SQS messages
    ...
    async recordSocialMediaCampaign(projectKey, platforms, postsCount) {
    ...
    async recordEmailOutreach(projectKey, emailCount, templateGeneration = false) {
        ...
        await this.recordCreditConsumption({
            ...
            description: `Email outreach to ${emailCount} recipients${templateGeneration ? " with template generation" : ""}`,
            usdConsumed: parseFloat(totalCost.toFixed(4)),
            metadata: { emailCount, templateGeneration },
        });
    }
    async recordBlockchainOperation(projectKey, operation, gasEstimate) {
    ...
    async recordPayment(payment) {
        ...
        console.log(`💳 Recorded ${payment.usdValue} USD payment for ${payment.projectKey}`);
    }
    // =============================================================================
    // FINANCIAL QUERIES
    // =============================================================================
    async getProjectFinancials(projectKey) {
        const tableName = process.env.PROJECT_FINANCIALS_TABLE || "project-financials";
    ...
    async getProjectPayments(projectKey, limit = 50) {
    ...
    async getCreditConsumptions(projectKey, fromDate, toDate, limit = 100) {
        const tableName = process.env.CREDIT_CONSUMPTIONS_TABLE || "credit-consumptions";
        let keyConditionExpression = "projectKey = :projectKey";
        const expressionValues = { ":projectKey": projectKey };
        if (fromDate && toDate) {
        ...
        const command = new QueryCommand({
            ...
            ExpressionAttributeNames: fromDate && toDate ? { "#timestamp": "timestamp" } : undefined,
    ...
    // STORAGE OPERATIONS
    // =============================================================================
    async storeCreditConsumption(consumption) {
        const tableName = process.env.CREDIT_CONSUMPTIONS_TABLE || "credit-consumptions";
    ...
    async updateProjectFinancials(projectKey, additionalConsumed = 0, additionalPaid = 0) {
        const tableName = process.env.PROJECT_FINANCIALS_TABLE || "project-financials";
        ...
        financials.totalDue = Math.max(0, financials.totalOverallUsed - financials.totalOverallPaid);
    ...
    // UTILITY METHODS
    // =============================================================================
    async getTopConsumingProjects(limit = 10) {
    ...
    async getConsumptionsByAgent(projectKey, agentName) {
