...
export class WebhookHandler {
    ...
    githubAgent;
    dataManager;
    constructor(githubAgent, dataManager) {
        this.octokit = new Octokit({ auth: process.env.GITHUB_TOKEN });
        this.githubAgent = githubAgent;
        this.dataManager = dataManager;
    }
    async registerWebhook(repoUrl) {
        ...
        try {
            ...
            const webhookUrl = `${process.env.WEBHOOK_BASE_URL || "https://your-domain.com"}/webhook/github`;
            ...
            console.error(`❌ Failed to register webhook for ${owner}/${repo}:`, error.message);
            throw error;
        }
    }
    ...
    async handlePushEvent(payload) {
        ...
        try {
            ...
            for (const commit of commits) {
                console.log(`🔄 Processing commit: ${commit.id.slice(0, 7)} - ${commit.message}`);
                ...
                if (significance > 0.2) {
                    ...
                    console.log(`   ✅ Significant commit (score: ${significance.toFixed(2)})`);
                    // Fetch detailed commit data
                    const [owner, repo] = repoName.split("/");
                    const detailedCommit = await this.fetchCommitDetails(owner, repo, commit.id);
                    // Update project data
                    await this.dataManager.updateProjectWithCommit(owner, repo, detailedCommit);
                }
                else {
                    console.log(`   ⏭️ Skipping minor commit (score: ${significance.toFixed(2)})`);
                }
            }
        }
        catch (error) {
