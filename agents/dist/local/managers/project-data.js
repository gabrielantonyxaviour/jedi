...
export class ProjectDataManager {
    ...
    async updateProjectWithCommit(owner, repo, commit) {
        const projectPath = path.join(this.localStoragePath, owner, repo);
        try {
            ...
            console.log(`✅ Updated project data for ${owner}/${repo} with commit ${commit.sha.slice(0, 7)}`);
        }
        catch (error) {
            console.error(`Failed to update project with commit:`, error);
        }
    }
    async updateProjectMetadata(owner, repo, commit) {
        const projectPath = path.join(this.localStoragePath, owner, repo);
        const analysisPath = path.join(projectPath, "complete-analysis.json");
        try {
            ...
            const changeAnalysis = {
                ...
                summary: `${commit.message.split("\n")[0]} | ${commit.files?.length || 0} files | +${commit.stats?.additions || 0} -${commit.stats?.deletions || 0}`,
            };
            analysis.activity.recentChanges.unshift(changeAnalysis);
            analysis.activity.recentChanges = analysis.activity.recentChanges.slice(0, 10);
